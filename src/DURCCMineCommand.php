...
class DURCCMineCommand extends Command{
	...
	foreach($db_struct as $db => $table_list){
		...
		foreach($table_list as $this_table_name => $column_data){
			...
			if($last_character == 's'){
				echo "Naming Convention Error: $db->$this_table_name ends in an 's'. But Laravel tried to do smart things with plurals, which prevents autogenerated code from being easy. So for now, just rename your table so that it does not end in 's'\n";
				echo "continuing, pretending\n\t$db->$this_table_name does not exist\n\n";
	...
	//CREATE TABLE comments should be included in the sourcecode comments
	//just to be clear where an specific mode is being generated from
	//and start the third  pass..
	foreach($db_struct as $this_db => $table_list){
		foreach($table_list as $this_table_name => $column_data){
			$my_object_name = strtolower($this_table_name);

			if(isset($new_struct[$this_db][$my_object_name]['table_name'])){
				//then we know that this was not an ignored table...


				$meta_sql = "SHOW CREATE TABLE $this_db.$this_table_name";
				$pdo = DB::connection()->getPdo();
				$result = $pdo->query($meta_sql,\PDO::FETCH_ASSOC);
				$table_array = $result->fetch();
				$create_table_sql = $table_array['Create Table'];

				//great, we have the table name, but not the table schema (database) that it is associated with.
				//a simple find and replace can fix that.

				$replace_me = "`$this_table_name`";
				$replace_with = "`$this_db`.`$this_table_name`";
	...
	//should there be a configuration file for these? not sure.
	$per_object_options_default = [
		'is_show_web_interface' => 1,
		'is_user_only_web_interface' => 0,
		'is_audited' => 0,
		'is_listed_in_webindex' => 0,
		];

	//this loop will create the presumed settings json file in an auto generated manner
	//for now we ignore the settings the user might previously have had... 
	//later we will overlay the user decisions on top of this auto-generated structure and re-save it..
	//which allows us to ensure that new tables are handled according the defaults set above, while previous user settings are always respected..
	...
	foreach($current_settings as $this_db => $table_list){
		foreach($table_list as $this_table_name => $setting_array){
			foreach($setting_array as $this_setting_key => $this_setting_value){
				//now we overwrite the autogenerated value that we had previously stored in config_settings_struct
				//with whatever the file previously had... 
				$config_settings_struct[$this_db][$this_table_name][$this_setting_key] = $this_setting_value;
			}
		}
	}
